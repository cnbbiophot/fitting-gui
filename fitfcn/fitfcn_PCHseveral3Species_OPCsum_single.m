function PCHnorm=fitfcn_PCHseveral3Species_OPCsum_single (param, tdata)
...
PCHnorm = simulationPCH_severalSpecies([epsilon1 epsilon2 epsilon3],[averageN1 averageN2 averageN3],...
    [F1 F2 F3], [Q1 Q2 Q3], length_K);
...
function PCH_total_norm = simulationPCH_severalSpecies(epsilon,averageN, F, Q,length_K)
    ...
    for S = 1:numberSpecies
        PCH_species(S,:) = generatePCH(length_K, epsilon(S), averageN(S), F(S), Q(S));        
    end
