function PCHnorm=fitfcn_PCHseveral4Species_OPCsum_global (param, indParam, x)
...
averageN4=param(indParam(:,4));
epsilon1=param(indParam(:,5));  
epsilon2=param(indParam(:,6));
epsilon3=param(indParam(:,7));
epsilon4=param(indParam(:,8));
F1=param(indParam(:,9));  
F2=param(indParam(:,10));
F3=param(indParam(:,11));
F4=param(indParam(:,12));
Q1=param(indParam(:,13));  
Q2=param(indParam(:,14));
Q3=param(indParam(:,15));
Q4=param(indParam(:,16));

PCHnorm = simulationPCH_severalSpecies_global([epsilon1 epsilon2 epsilon3 epsilon4],[averageN1 averageN2 averageN3 averageN4],...
    [F1 F2 F3 F4], [Q1 Q2 Q3 Q4], length_K, number_sets);
...
function PCH_total_norm = simulationPCH_severalSpecies_global(epsilon,averageN,F,Q,length_K, number_sets)
    ...
    for S = 1:numberSpecies
        PCH_species(S,:) = generatePCH_severalSpecies_global(length_K, epsilon(:,S), averageN(:,S), F(:,S), Q(:,S), number_sets);        
    end
