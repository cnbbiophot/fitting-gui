function fVes=fitfcn_partitionCoefficient_M3_melo11_Boltzmann_single(param, tdata)
...
% DO NOT USE ZERO X (LIPID) VALUES 
%
...
xm=(Kp.*LAccessible)./(W+Kp.*LAccessible)*xmax; % Mole fraction of labeled-proteins bound to vesicles
...
function P_k_k_average = P_k_k_average(x, x_average) 

        P_k_k_average = x_average.^x ./ (x_average + 1).^(x + 1);
