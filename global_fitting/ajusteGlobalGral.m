function [paramfitY, allparam, J]=ajusteGlobalGral(dataCell, varGlobals, paramsIni, varFix, funcAjuste)
...
for n=1:numParams
    ...
    valorLB(n) = 0;  % Esto se podrá ajustar cuando se inserte en la GUI
    valorUB(n) = Inf;   % Esto se podrá ajustar cuando se inserte en la GUI
...
options=optimset (oldoptions, 'Display', 'final-detailed', 'Jacobian', 'on'); %Opciones de ajuste para lsqnonlin. Jacobiano activado para reducir el nº de iteraciones 

valorParamVarIni = paramsIni(indparamvariables);

[paramfitY, allparam,~,~,~,~,~,J] = ...
    ajusta_lsqnonlin_global(valorParamVarIni, LB, UB, ...
    options, data_all, indparamvariables, indparamfijos, valorparamfijos,...
    numPtosxCurva, varGlobals, indParamsGlobal2Indep, varFix, funcAjuste);

% Plot de los ajustes
funcAjuste = str2func(funcAjuste);
G = funcAjuste (allparam', indParamsGlobal2Indep, [dataMat(:,1) dsid]);
tdata = dataMat(:,1)*1E3; % Se pone la señal en ms

figure
Fit=dataMat(:,2);

for s = 1:numCurvas     % Plot separated curves   
    G_separated = G( (s-1)*numPtosxCurva(s)+1 : sum(numPtosxCurva(1:s)) );
    tdata_separated = tdata( (s-1)*numPtosxCurva(s)+1 : sum(numPtosxCurva(1:s)) );
    FuncPlot = semilogx(tdata_separated,G_separated,'k');
    hold on
end

FitPlot = semilogx(tdata,Fit, 'r+');
xlabel('Tau lag (ms)');
ylabel('ACF and fit');
legend([FuncPlot, FitPlot],'Fit','Data');
title('Fit');
